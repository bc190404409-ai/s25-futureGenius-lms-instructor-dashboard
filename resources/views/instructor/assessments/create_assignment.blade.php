...
<form action="{{ route('instructor.assessments.store') }}" method="POST">
    ...
    <div class="mb-4 p-3 border rounded">
        ...
        <textarea name="questions[{{ $i }}][question_text]" placeholder="Describe the assignment task" class="border rounded px-3 py-2 w-full mb-2" required></textarea>
        <label>Marks</label>
        <input type="number" name="questions[{{ $i }}][marks]" min="0" class="border rounded px-3 py-2 w-full mb-1" required>
    </div>
    @endfor

    <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded">Save Assignment</button>
</form>
@endsection
